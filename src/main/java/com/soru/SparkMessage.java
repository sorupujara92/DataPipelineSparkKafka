...
public class SparkMessage {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    lines.foreachRDD(javaRdd -> {
      ...
      for (String str : list) {
        KafkaMessage kafkaMessage = mapper.readValue(str, KafkaMessage.class);
        JavaRDD<KafkaMessage> rdd = streamingContext.sparkContext()
            .parallelize(Arrays.asList(kafkaMessage));
        javaFunctions(rdd).writerBuilder("kafkamessage", "pubsubmessage",
                mapToRow(KafkaMessage.class))
            .saveToCassandra();
      }
    });
